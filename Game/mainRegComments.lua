...
function love.draw()
	...
	elseif state == "highScores" then
		...
		if first_time_played == true and tempTimes ~= 0 then
...
function love.update(dt)
	...
	elseif state == "highScores" then
		...
		ChangeGravity() --function to randomly change the gravity
		if objects.ball.body:getX() < 0 then -- if the ball goes off the screen to the left GAMEOVER
		  state = "gameOver"
		elseif objects.ball.body:getX() > WINDOW_W-10 then --do not let the ball go off to the right
		...
		if love.keyboard.isDown("right") and x < 50 then --put a max velocity on how fast you can go right and left
			objects.ball.body:applyForce(5, 0) 
		elseif love.keyboard.isDown("left") and x > -50 then
			objects.ball.body:applyForce(-5, 0)
		end

		speedInc = speedInc - 0.05 --increment the speed in the negative direction
		for j = 1, #blocks do -- for all the blocks in the table set the linear veocity 
		  blocks[j].body:setLinearVelocity(-100+speedInc, 0)
		end
		local xB = objects.ball.body:getX() --get the X and Y coordinates of the ball
		local yB = objects.ball.body:getY()


		for pi = 1, #pits do --for all of the pits apply velocity
		 local xV, yV = pits[pi].body:getLinearVelocity() --linear velocity of the pit
		 local xP = pits[pi].body:getX() --x val of the pit
		 local yP = pits[pi].body:getY() --y val of the pit
		...
		--set Linear velocity of the pit
		  pits[pi].body:setLinearVelocity(-100+speedInc, -yV) --coneract the force of gravity on the pit 
		end
		end -- end for


		random = love.math.random(1000) --generate a random number between 1 AND 1000
		-- if its divisible by 100, a block hasn't previously been made in the past 25 frames
		-- then spawn a block
		if random % 100 == 0 and blockMade ~= true and bCount > 25 then
		  ...
		  pitMade = false
		  bCount = 0
		-- if its divisible by 50, a pit hasn't been previously been made in the past 35 frame
		-- then spawn a pit
		...
		timeGame = love.timer.getTime( ) --get the time of the game
...
function makeBlock() 
  ...
  local bottom = WINDOW_H-50-height/2 - 2 --Y value for bottom position
  local top = 50 + height/2 --Y value for top position
  local gravBlock = bottom --Y val variable to position Block
  
  local width = love.math.random(40, 60) -- randomize width
  
  if GravPos == 0 then --set the Y value for the block depending on the Gravity Position
    gravBlock = bottom
  elseif GravPos == 1 then
    gravBlock = top
  end 
  
  blocks[count] = {} --create a block object
  blocks[count].body = love.physics.newBody(world, WINDOW_W, gravBlock, "dynamic") --create the body 
  blocks[count].shape = love.physics.newRectangleShape(width, height) -- create the shape
  blocks[count].fixture = love.physics.newFixture(blocks[count].body, blocks[count].shape, 5) -- bind the shape to the body
  count = count + 1
  if count == 30 then pitCount = 1 end --stop the table of blocks from getting infinately too big
 
end

function makePit() 
  pBot = WINDOW_H-50/2 --Y val for bottom 
  pTop = 50/2 --y val for top
  gravPit = bottom --Y val variable to position pit
  local widthP = love.math.random(40, 65) -- randomize width between 40 and 65
  widths[pitCount] = widthP -- create a table of widths for collision dectection
  if GravPos == 0 then --set the Y value of the pit based on the gravity
    gravPit = pBot 
  elseif GravPos == 1 then
    gravPit = pTop
  end
  pits[pitCount] = {} --create the pit 
  pits[pitCount].body = love.physics.newBody(world, WINDOW_W, gravPit, "kinematic") --create the body 
  -- kinematic can interact with dynamic objects only - thats why it can be overlayed on the ground
  pits[pitCount].shape = love.physics.newRectangleShape(widthP, 50) --set width and height of the rectangle
  pits[pitCount].fixture = love.physics.newFixture(pits[pitCount].body, pits[pitCount].shape, 5) --bind rectangle to body
  pitCount = pitCount + 1
  if pitCount == 30 then pitCount = 1 end
...
function ChangeGravity()
  ...
  if gravNum % 1000 == 0 then --if gravNum is divisible by 1000 
    if GravPos == 1 then --if the position is 1(top) make it bottom and vice versa
       GravPos = 0
       world:setGravity(0,Grav)
    else 
...
function ReadScores(filename, list)
	...
	scorefile = love.filesystem.newFile("scores.txt")
	scorefile:open("r")
	data = scorefile:read()
	--io.input(filename)
  ...
  --[[
	...
	for i = 1, #list do
	  io.write(list[i].score, ' ', list[i].name, ' ', list[i].times_played, '\n')
	end
	end
--]]
